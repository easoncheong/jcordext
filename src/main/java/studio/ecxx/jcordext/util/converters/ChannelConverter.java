...
public class ChannelConverter extends DiscordConverter<Channel> {

    public ChannelConverter(DiscordApi api) {this.api = api;}
     ...
     */
    @Override
    public Channel convert(Channel context, String input) {

        String idString = input.replaceAll("[\\\\<>@#&!]", "");
        Optional<Channel> channel = api.getChannelById(idString);
